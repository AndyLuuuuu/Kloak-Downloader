...
DataStream.prototype.readFloat32 = function(e) {
  var v = this._dataView.getFloat32(
    this.position,
    e == null ? this.endianness : e
  )
  this.position += 4
...
DataStream.prototype.readFloat64 = function(e) {
  var v = this._dataView.getFloat64(
    this.position,
    e == null ? this.endianness : e
  )
  this.position += 8
...
*/
DataStream.prototype.writeUCS2String = function(
  str,
  endianness,
  lengthOverride
) {
...
MultiBufferStream.prototype.logBufferLevel = function(info) {
  ...
  for (i = 0; i < this.buffers.length; i++) {
    ...
    if (i === 0) {
      ...
      range.start = buffer.fileStart
      bufferedString +=
        ranges[ranges.length - 1].end - 1 + '], [' + range.start + '-'
      range.end = buffer.fileStart + buffer.byteLength
      ranges.push(range)
    }
 ...
 */
MultiBufferStream.prototype.findPosition = function(
  fromStart,
  filePosition,
  markAsUsed
) {
  ...
  if (index !== -1) {
    ...
    if (abuffer.fileStart + abuffer.byteLength >= filePosition) {
      Log.debug(
        'MultiBufferStream',
        'Found position in existing buffer #' + index
      )
      return index
    } else {
      return -1
 ...
 */
MultiBufferStream.prototype.seek = function(
  filePosition,
  fromStart,
  markAsUsed
) {
  ...
  if (index !== -1) {
    ...
    Log.debug(
      'MultiBufferStream',
      'Position ' + filePosition + ' not found in buffered data'
    )
    return false
  }
}
  ...
  classes.ES_Descriptor.prototype.getAudioConfig = function(stream) {
    ...
    if (dsi && dsi.data) {
      ...
      if (audioObjectType === 31 && dsi.data.length >= 2) {
        audioObjectType =
          32 + ((dsi.data[0] & 0x7) << 3) + ((dsi.data[1] & 0xe0) >> 5)
      }
...
var BoxParser = {
  ...
  createSampleEntryCtor: function(mediaType, type, parseMethod, subBoxNames) {
    ...
    BoxParser[type + 'SampleEntry'].prototype = new BoxParser[
      mediaType + 'SampleEntry'
    ]()
    if (parseMethod)
  ...
  createTrackGroupCtor: function(type, parseMethod) {
    ...
    ].prototype = new BoxParser.TrackGroupTypeBox()
    if (parseMethod)
  ...
  createUUIDBox: function(uuid, isFullBox, isContainerBox, parseMethod) {
    ...
    BoxParser.UUIDBoxes[uuid].prototype = isFullBox
...
BoxParser.parseOneBox = function(stream, headerOnly, parentSize) {
  ...
  if (size == 1) {
    ...
    if (parentSize) {
      ...
      if (type !== 'mdat') {
        Log.error(
          'BoxParser',
          "Unlimited box size not supported for type: '" + type + "'"
        )
  ...
  if (start + size > stream.getEndPosition()) {
    ...
    Log.info(
      'BoxParser',
      "Not enough data in stream to parse the entire '" + type + "' box"
    )
    return {
      code: BoxParser.ERR_NOT_ENOUGH_DATA,
      type: type,
  ...
  while (stream.getPosition() < this.start + this.size) {
    ...
    if (ret.code === BoxParser.OK) {
      ...
      if (this.subBoxNames && this.subBoxNames.indexOf(box.type) != -1) {
        this[this.subBoxNames[this.subBoxNames.indexOf(box.type)] + 's'].push(
          box
        )
      } else {
...
BoxParser.createEncryptedSampleEntryCtor(
  BoxParser.SAMPLE_ENTRY_TYPE_VISUAL,
  'encv'
)
BoxParser.createEncryptedSampleEntryCtor(
...
BoxParser.createEncryptedSampleEntryCtor(
  BoxParser.SAMPLE_ENTRY_TYPE_SYSTEM,
  'encs'
)
BoxParser.createEncryptedSampleEntryCtor(
...
BoxParser.createSampleGroupCtor('rash', function(stream) {
  ...
  if (
    ...
    2 +
      (this.operation_point_count === 1 ? 2 : this.operation_point_count * 6) +
      9
  ) {
    Log.warn(
      'BoxParser',
...
BoxParser.createFullBoxCtor('tenc', function(stream) {
  ...
  if (this.default_isProtected === 1 && this.default_Per_Sample_IV_Size === 0) {
    ...
    this.default_constant_IV = stream.readUint8Array(
...
BoxParser.createFullBoxCtor('tfra', function(stream) {
  ...
  for (var i = 0; i < number_of_entries; i++) {
    ...
    this.traf_number = stream[
      'readUint' + 8 * (this.length_size_of_traf_num + 1)
    ]()
    this.trun_number = stream[
      'readUint' + 8 * (this.length_size_of_trun_num + 1)
    ]()
    this.sample_number = stream[
      'readUint' + 8 * (this.length_size_of_sample_num + 1)
    ]()
  }
})
...
BoxParser.trefBox.prototype.parse = function(stream) {
  ...
  while (stream.getPosition() < this.start + this.size) {
    ...
    if (ret.code === BoxParser.OK) {
      box = new BoxParser.TrackReferenceTypeBox(
        ret.type,
        ret.size,
        ret.hdr_size,
        ret.start
...
BoxParser.createFullBoxCtor('vpcC', function(stream) {
  ...
  if (this.version === 1) {
    ...
    this.codecIntializationData = stream.readUint8Array(
...
BoxParser.mp4aSampleEntry.prototype.getCodec = function() {
  ...
  if (this.esds && this.esds.esd) {
    ...
    return (
      baseCodec + '.' + BoxParser.decimalToHex(oti) + (dsi ? '.' + dsi : '')
    )
  } else {
    return baseCodec
  }
...
BoxParser.sidxBox.prototype.write = function(stream) {
  ...
  for (var i = 0; i < this.references.length; i++) {
    ...
    stream.writeUint32(
      (ref.starts_with_SAP << 31) | (ref.SAP_type << 28) | ref.SAP_delta_time
    )
  }
}
...
BoxParser.stscBox.prototype.unpack = function(samples) {
  ...
  for (i = 0; i < this.first_chunk.length; i++) {
    for (
      j = 0;
      j <
      (i + 1 < this.first_chunk.length ? this.first_chunk[i + 1] : Infinity);
      j++
    ) {
      m++
...
BoxParser.boxEqualFields = function(box_a, box_b) {
  ...
  for (prop in box_a) {
    if (BoxParser.DIFF_BOXES_PROP_NAMES.indexOf(prop) > -1) {
      ...
      box_a[prop] instanceof BoxParser.Box ||
      box_b[prop] instanceof BoxParser.Box
    ) {
      continue
    } else if (
      typeof box_a[prop] === 'undefined' ||
      typeof box_b[prop] === 'undefined'
    ) {
      continue
    } else if (
      typeof box_a[prop] === 'function' ||
      typeof box_b[prop] === 'function'
    ) {
      continue
    } else if (
...
VTTin4Parser.prototype.getText = function(startTime, endTime, data) {
  ...
  for (var i = 0; i < cues.length; i++) {
    var cueIn4 = cues[i]
    string +=
      secToTimestamp(startTime) + ' --> ' + secToTimestamp(endTime) + '\r\n'
...
XMLSubtitlein4Parser.prototype.parseSample = function(sample) {
  ...
  if (typeof DOMParser !== 'undefined') {
    res.document = new DOMParser().parseFromString(
      res.documentString,
      'application/xml'
    )
  }
...
ISOFile.prototype.checkBuffer = function(ab) {
  ...
  if (ab.byteLength === 0) {
    Log.warn(
      'ISOFile',
      'Ignoring empty buffer (fileStart: ' + ab.fileStart + ')'
    )
...
ISOFile.prototype.getInfo = function() {
  ...
  if (this.moov) {
    ...
    for (i = 0; i < this.moov.traks.length; i++) {
      ...
      track.kind =
        trak.udta && trak.udta.kinds.length
          ? trak.udta.kinds[0]
          : { schemeURI: '', value: '' }
      ...
      track.bitrate =
        (track.size * 8 * track.timescale) / track.samples_duration
  ...
  if (this.isFragmentationInitialized && this.onSegment !== null) {
    for (i = 0; i < this.fragmentedTracks.length; i++) {
      ...
      or because the file is fragmented and the moof for that sample has been received */
        ...
        var result = this.createFragment(
          fragTrak.id,
          trak.nextSample,
...
ISOFile.prototype.seekTrack = function(time, useRap, trak) {
  ...
  while (
    trak.samples[seek_sample_num].alreadyRead ===
    trak.samples[seek_sample_num].size
  ) {
  ...
  seek_offset =
    trak.samples[seek_sample_num].offset +
    trak.samples[seek_sample_num].alreadyRead
  Log.info(
    'ISOFile',
    'Seeking to ' +
...
ISOFile.prototype.addTrack = function(_options) {
  ...
  minf
    .add('dinf')
    .add('dref')
    .addEntry(new BoxParser['url Box']().set('flags', 0x1))
  ...
  this.moov.mvex
    ...
    .set(
      ...
      options.default_sample_description_index || 1
    )
...
ISOFile.prototype.addSample = function(track_id, data, _options) {
  ...
  sample.description_index = options.sample_description_index
    ...
    : 0
  sample.description =
    trak.mdia.minf.stbl.stsd.entries[sample.description_index]
  sample.data = data
...
ISOFile.initSampleGroups = function(trak, traf, sbgps, trak_sgpds, traf_sgpds) {
  ...
  for (k = 0; k < sbgps.length; k++) {
    sample_group_key =
      sbgps[k].grouping_type + '/' + sbgps[k].grouping_type_parameter
...
ISOFile.setSampleGroupProperties = function(
  trak,
  sample,
  sample_number,
  sample_groups_info
  ...
  for (k in sample_groups_info) {
    ...
    if (sample_number >= sample_groups_info[k].last_sample_in_run) {
      ...
      if (
        sample_groups_info[k].entry_index <=
        sample_groups_info[k].sbgp.entries.length - 1
      ) {
        ...
        if (description && description.version >= 2) {
          if (description.default_group_description_index > 0) {
            sample.sample_groups[k].description =
              description.entries[
                description.default_group_description_index - 1
              ]
          }
        }
      }
...
ISOFile.prototype.buildTrakSampleLists = function(trak) {
  ...
  for (j = 0; j < stsz.sample_sizes.length; j++) {
    ...
    sample.description_index =
      stsc.sample_description_index[sample.chunk_run_index] - 1
    ...
    if (stss) {
      ...
      if (subs) {
        if (
          subs.entries[subs_entry_index].sample_delta +
            last_subs_sample_index ==
          j + 1
        ) {
    ...
    if (subs) {
      if (
        subs.entries[subs_entry_index].sample_delta + last_subs_sample_index ==
        j
      ) {
        sample.subsamples = subs.entries[subs_entry_index].subsamples
        last_subs_sample_index += subs.entries[subs_entry_index].sample_delta
  ...
  if (j > 0) {
    trak.samples[j - 1].duration = Math.max(
      trak.mdia.mdhd.duration - trak.samples[j - 1].dts,
      0
    )
    trak.samples_duration += trak.samples[j - 1].duration
  }
    ...
    if (box.type == 'moof') {
      ...
      for (i = 0; i < moof.trafs.length; i++) {
        ...
        if (traf.tfhd.flags & BoxParser.TFHD_FLAG_SAMPLE_DESC) {
          default_sample_description_index =
            traf.tfhd.default_sample_description_index
        } else {
          default_sample_description_index = trex
        ...
        if (traf.sbgps.length > 0) {
          ISOFile.initSampleGroups(
            trak,
            traf,
            traf.sbgps,
            trak.mdia.minf.stbl.sgpds,
            traf.sgpds
          )
        }
        for (j = 0; j < traf.truns.length; j++) {
          ...
          for (k = 0; k < trun.sample_count; k++) {
            ...
            if (trun.flags & BoxParser.TRUN_FLAGS_FLAGS) {
              ...
              k === 0 &&
              trun.flags & BoxParser.TRUN_FLAGS_FIRST_FLAG
            ) {
            ...
            var bdop =
              traf.tfhd.flags & BoxParser.TFHD_FLAG_BASE_DATA_OFFSET
                ? true
                : false
            var dbim =
              traf.tfhd.flags & BoxParser.TFHD_FLAG_DEFAULT_BASE_IS_MOOF
                ...
                : false
            var dop =
              trun.flags & BoxParser.TRUN_FLAGS_DATA_OFFSET ? true : false
            var bdo = 0
            if (!bdop) {
              if (!dbim) {
                if (j === 0) {
...
ISOFile.prototype.getSample = function(trak, sampleNum) {
  ...
  var index = this.stream.findPosition(
    true,
    sample.offset + sample.alreadyRead,
    false
  )
  if (index > -1) {
    ...
    for (i = 0; i < meta.iloc.items.length; i++) {
      ...
      for (j = 0; j < itemloc.extents.length; j++) {
        ...
        item.extents[j].offset =
          itemloc.extents[j].extent_offset + itemloc.base_offset
...
ISOFile.prototype.getItem = function(item_id) {
  ...
  for (var i = 0; i < item.extents.length; i++) {
    ...
    if (extent.alreadyRead === extent.length) {
      ...
      var index = this.stream.findPosition(
        true,
        extent.offset + extent.alreadyRead,
        false
      )
      if (index > -1) {
        buffer = this.stream.buffers[index]
...
ISOFile.prototype.createFragment = function(track_id, sampleNumber, stream_) {
  ...
  if (sample == null) {
    ...
    if (this.nextSeekPosition) {
      ...
      this.nextSeekPosition =
        trak.samples[sampleNumber].offset + sample.alreadyRead
    }
    return null
  }
...
ISOFile.writeInitializationSegment = function(
  ftyp,
  moov,
  total_duration,
  sample_duration
...
ISOFile.prototype.initializeSegmentation = function() {
  ...
  for (i = 0; i < this.fragmentedTracks.length; i++) {
    ...
    seg.buffer = ISOFile.writeInitializationSegment(
      this.ftyp,
      moov,
      this.moov.mvex && this.moov.mvex.mehd
        ? this.moov.mvex.mehd.fragment_duration
        : undefined,
      this.moov.traks[i].samples.length > 0
        ? this.moov.traks[i].samples[0].duration
        : 0
    )
    initSegs.push(seg)
  }
