  ...
  goog.writeScriptTag_ = function(src, opt_sourceText) {
    if (goog.inHtmlDocument_()) {
      ...
      if (opt_sourceText === undefined)
        ...
        doc.write(
          '<script type="text/javascript">' + opt_sourceText + '</' + 'script>'
        )
      return true
    } else return false
  }
...
goog.string.caseInsensitiveStartsWith = function(str, prefix) {
  return (
    goog.string.caseInsensitiveCompare(prefix, str.substr(0, prefix.length)) ==
    0
  )
}
...
goog.string.htmlEscape = function(str, opt_isLikelyToContainHtmlChars) {
  if (opt_isLikelyToContainHtmlChars) {
    ...
    if (str.indexOf('&') != -1) str = str.replace(goog.string.AMP_RE_, '&amp;')
...
goog.string.compareVersions = function(version1, version2) {
  ...
  for (var subIdx = 0; order == 0 && subIdx < subCount; subIdx++) {
    ...
    do {
      ...
      order =
        goog.string.compareElements_(v1CompNum, v2CompNum) ||
...
goog.string.capitalize = function(str) {
  return (
    String(str.charAt(0)).toUpperCase() + String(str.substr(1)).toLowerCase()
  )
}
...
goog.userAgent.isX11_ = function() {
  var navigator = goog.userAgent.getNavigator()
  return (
    !!navigator && goog.string.contains(navigator['appVersion'] || '', 'X11')
  )
}
...
goog.uri.utils.findParam_ = function(
  ...
  while (
    (index = uri.indexOf(keyEncoded, index)) >= 0 &&
...
goog.math.sampleVariance = function(var_args) {
  ...
  var variance =
    goog.math.sum.apply(
      null,
      goog.array.map(arguments, function(val) {
...
goog.Uri.prototype.setScheme = function(newScheme, opt_decode) {
  ...
  this.scheme_ = opt_decode
    ? goog.Uri.decodeOrEmpty_(newScheme, true)
    : newScheme
...
goog.Uri.prototype.setDomain = function(newDomain, opt_decode) {
  ...
  this.domain_ = opt_decode
    ? goog.Uri.decodeOrEmpty_(newDomain, true)
    : newDomain
  return this
}
...
if (shaka.asserts.ENABLE_ASSERTS) {
  ...
  shaka.asserts.notImplemented = console.assert.bind(
    console,
    0 == 1,
    'Not implemented.'
  )
  shaka.asserts.unreachable = console.assert.bind(
    console,
    0 == 1,
    'Unreachable reached.'
  )
...
shaka.util.Uint8ArrayUtils.toBase64 = function(arr, opt_padding) {
  return shaka.util.StringUtils.toBase64(
    shaka.util.Uint8ArrayUtils.toString(arr),
    opt_padding
  )
}
...
shaka.util.Pssh = function(psshBox) {
  ...
  try {
    ...
    shaka.log.warning(
      'PSSH parse failure!  Some data may be missing or ' + 'incorrect.'
    )
  }
}
...
shaka.util.LanguageUtils.match = function(fuzz, preference, candidate) {
  ...
  if (
    fuzz >= shaka.util.LanguageUtils.MatchType.BASE_LANGUAGE_OKAY &&
    candidate == preference.split('-')[0]
  )
    return true
  if (
    fuzz >= shaka.util.LanguageUtils.MatchType.OTHER_SUB_LANGUAGE_OKAY &&
...
shaka.dash.mpd.Mpd.prototype.parse = function(parent, elem) {
  ...
  this.minBufferTime = mpd.parseAttr_(
    elem,
    'minBufferTime',
    mpd.parseDuration_,
    this.minBufferTime
  )
  this.minUpdatePeriod = mpd.parseAttr_(
    elem,
    'minimumUpdatePeriod',
    mpd.parseDuration_,
    this.minUpdatePeriod
  )
  if (this.minUpdatePeriod == 0) this.minUpdatePeriod = 10
  this.availabilityStartTime = mpd.parseAttr_(
    elem,
    'availabilityStartTime',
    mpd.parseDate_,
    this.availabilityStartTime
  )
  this.timeShiftBufferDepth = mpd.parseAttr_(
    elem,
    'timeShiftBufferDepth',
    mpd.parseDuration_,
    this.timeShiftBufferDepth
  )
  this.suggestedPresentationDelay = mpd.parseAttr_(
    elem,
...
shaka.dash.mpd.AdaptationSet.prototype.parse = function(parent, elem) {
  ...
  var baseUrl = mpd.parseChild_(this, elem, mpd.BaseUrl)
  this.baseUrl = mpd.resolveUrl_(parent.baseUrl, baseUrl ? baseUrl.url : null)
  this.contentProtections = mpd.parseChildren_(
    this,
    elem,
...
shaka.dash.mpd.SegmentBase.prototype.parse = function(parent, elem) {
  ...
  this.representationIndex =
    ...
    this.representationIndex
  this.initialization =
    mpd.parseChild_(this, elem, mpd.Initialization) || this.initialization
}
...
shaka.dash.mpd.SegmentList.prototype.parse = function(parent, elem) {
  ...
  this.initialization =
    mpd.parseChild_(this, elem, mpd.Initialization) || this.initialization
  this.segmentUrls =
    mpd.parseChildren_(this, elem, mpd.SegmentUrl) || this.segmentUrls
}
...
shaka.dash.mpd.SegmentTemplate.prototype.parse = function(parent, elem) {
  ...
  this.timeline =
    mpd.parseChild_(this, elem, mpd.SegmentTimeline) || this.timeline
}
...
shaka.media.SegmentIndex.prototype.merge = function(segmentIndex) {
  if (this.timestampCorrection_ != segmentIndex.timestampCorrection_) {
    ...
    shaka.log.v1(
      'Shifting new SegmentReferences by',
      delta,
  ...
  if (
    segmentIndex.last().endTime != null &&
...
shaka.media.SegmentIndex.prototype.correct = function(timestampCorrection) {
  ...
  this.references_ = shaka.media.SegmentIndex.shift_(
    this.references_,
    timestampCorrection
  )
  this.assertCorrectReferences_()
...
shaka.media.SegmentIndex.prototype.assertCorrectReferences_ = function() {
  ...
  shaka.asserts.assert(
    references.every(function(item, index) {
      ...
      if (index != 0)
        result =
          item.startTime >= previousItem.startTime &&
...
shaka.media.ISegmentIndexParser.prototype.parse = function(
  initSegmentData,
  indexData,
  indexOffset
) {}
...
shaka.media.IsobmffSegmentIndexParser.prototype.parse = function(
  initSegmentData,
  indexData,
  indexOffset
) {
...
shaka.util.AjaxRequest.prototype.handleOfflineUri_ = function() {
  ...
  return db
    ...
    .then(
      shaka.util.TypedBind(this, function(data) {
        var xhr = JSON.parse(JSON.stringify(new XMLHttpRequest()))
        xhr.response = data
        var promise = this.promise_
        promise.resolve(xhr)
    ...
    ['catch'](
      shaka.util.TypedBind(this, function(e) {
        db.closeDatabaseConnection()
        this.destroy_()
...
shaka.util.RangeRequest.prototype.send = function() {
  return this.sendInternal().then(
    shaka.util.TypedBind(this, function(xhr) {
...
shaka.media.StreamInfo.prototype.getSegmentIndex = function() {
  ...
  p = p.then(
    shaka.util.TypedBind(this, function() {
      ...
      if (mp4)
        indexParser = new shaka.media.IsobmffSegmentIndexParser(this.mediaUrl)
      else indexParser = new shaka.media.WebmSegmentIndexParser(this.mediaUrl)
      shaka.asserts.assert(indexParser)
      var segmentInitializationDataView = this.segmentInitializationData
      ...
      var references = indexParser.parse(
        segmentInitializationDataView,
        segmentIndexDataView,
        indexOffset
      )
      if (!references) {
...
shaka.media.StreamInfo.prototype.fetchSegmentIndex_ = function() {
  ...
  return this.segmentIndexInfo.fetch().then(
    shaka.util.TypedBind(this, function(data) {
...
shaka.media.StreamInfo.prototype.fetchSegmentInitialization_ = function() {
  ...
  return this.segmentInitializationInfo.fetch().then(
    shaka.util.TypedBind(this, function(data) {
...
shaka.dash.MpdProcessor.prototype.calculateDurations_ = function(mpd) {
  ...
  if (
    isSet(mpd.mediaPresentationDuration) &&
  ...
  if (isSet(mpd.mediaPresentationDuration)) {
    if (mpd.mediaPresentationDuration != totalDuration)
      shaka.log.warning(
        '@mediaPresentationDuration does not match the total ' +
    ...
    if (totalDurationIncludesAllPeriods) {
      ...
      shaka.asserts.assert(
        totalDuration == finalPeriod.start + finalPeriod.duration
      )
      ...
      shaka.log.warning(
        'Some Periods may not have valid start times ' +
...
shaka.dash.MpdProcessor.prototype.createManifestInfo_ = function(mpd) {
  ...
  this.manifestInfo.minBufferTime =
    mpd.minBufferTime || shaka.dash.MpdProcessor.DEFAULT_MIN_BUFFER_TIME
  for (var i = 0; i < mpd.periods.length; ++i) {
    var period = mpd.periods[i]
    ...
    for (var j = 0; j < period.adaptationSets.length; ++j) {
      ...
      for (var k = 0; k < adaptationSet.representations.length; ++k) {
        ...
        if (streamInfo.segmentIndex && streamInfo.segmentIndex.length() > 0)
          maxLastEndTime = Math.max(
            maxLastEndTime,
            streamInfo.segmentIndex.last().endTime
          )
      }
...
shaka.dash.MpdProcessor.prototype.getDrmSchemeInfos_ = function(
  representation
) {
  var drmSchemes = []
  return drmSchemes
...
shaka.dash.MpdProcessor.prototype.buildStreamInfoFromSegmentBase_ = function(
  segmentBase,
  streamInfo
) {
  ...
  var hasSegmentIndexMetadata =
    ...
    (segmentBase.representationIndex && segmentBase.representationIndex.range)
...
shaka.dash.MpdProcessor.prototype.buildStreamInfoFromSegmentList_ = function(
  segmentList,
  streamInfo
) {
  ...
  shaka.log.debug(
    'Generated SegmentIndex from SegmentList',
    streamInfo.segmentIndex
  )
  return true
}
shaka.dash.MpdProcessor.prototype.buildStreamInfoFromSegmentTemplate_ = function(
  mpd,
  period,
  representation,
  streamInfo
...
shaka.dash.MpdProcessor.prototype.buildStreamInfoFromIndexUrlTemplate_ = function(
  representation,
  streamInfo
) {
  shaka.asserts.assert(representation.segmentTemplate)
...
shaka.dash.MpdProcessor.prototype.buildStreamInfoFromSegmentTimeline_ = function(
  mpd,
  period,
  representation,
  streamInfo
  ...
  if (mpd.minUpdatePeriod && timeline.length > 0) {
    ...
    earliestAvailableTimestamp =
      timeline[index].start / segmentTemplate.timescale - timeShiftBufferDepth
  }
  var references = []
  ...
  streamInfo.segmentIndex = new shaka.media.SegmentIndex(references)
  shaka.log.debug(
    'Generated SegmentIndex from SegmentTimeline',
    streamInfo.segmentIndex
  )
  return true
}
  ...
  for (var i = 0; i < timePoints.length; ++i) {
    ...
    for (var j = 0; j <= repeat; ++j) {
      ...
      if (timeline.length > 0 && startTime != lastEndTime) {
        ...
        if (
          Math.abs(delta / segmentTemplate.timescale) >=
...
shaka.dash.MpdProcessor.prototype.buildStreamInfoFromSegmentDuration_ = function(
  mpd,
  period,
  representation,
  streamInfo
  ...
  shaka.asserts.assert(
    earliestSegmentNumber === undefined || earliestSegmentNumber >= 0
  )
  var numSegmentsFromCurrentSegment = 0
  if (earliestSegmentNumber >= 0) {
    numSegmentsFromCurrentSegment = this.computeOptimalSegmentIndexSize_(
      mpd,
      period,
      segmentTemplate
    )
  ...
  streamInfo.segmentIndex = new shaka.media.SegmentIndex(references)
  shaka.log.debug(
    'Generated SegmentIndex from segment duration',
    streamInfo.segmentIndex
  )
  return true
}
...
shaka.dash.MpdProcessor.prototype.computeAvailableSegmentRange_ = function(
  ...
  var availabilityStartTime =
    mpd.availabilityStartTime != null ? mpd.availabilityStartTime : currentTime
  ...
  var earliestAvailableTimestamp =
  ...
  var latestAvailableSegmentStartTime =
    ...
    scaledSegmentDuration
  var bestAvailableTimestamp =
    latestAvailableSegmentStartTime - suggestedPresentationDelay - minBufferTime
